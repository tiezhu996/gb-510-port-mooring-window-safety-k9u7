...
type SafetyClearanceHandler struct {
	service service.SafetyClearanceService
}

func NewSafetyClearanceHandler(s service.SafetyClearanceService) *SafetyClearanceHandler {
	return &SafetyClearanceHandler{service: s}
}

func (h *SafetyClearanceHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/clearance")
	resource.GET("", h.list)
...
func (h *SafetyClearanceHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *SafetyClearanceHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *SafetyClearanceHandler) create(c *gin.Context) {
	var input dto.CreateSafetyClearance
...
func (h *SafetyClearanceHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateSafetyClearance
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *SafetyClearanceHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *SafetyClearanceHandler) remove(c *gin.Context) {
