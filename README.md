...
- 靠泊放行闸门：靠泊任务从计划或进港推进到已系泊时，强制核对同一泊位下已放行且已生效的安全许可、其风浪窗口处于 `safe` 且窗口版本与许可一致；任一条件不满足即保持原状态并返回阻断项清单。闸门评估、状态更新与审计写入在同一事务中提交，窗口在推进期间变化或重复、并发推进都不会放行，失败不会改动任务、许可、窗口和审计记录。`GET /api/vessels/:id/gate` 实时重算检查结果，靠泊页的放行闸门面板据此展示并通过刷新回读。
